...
namespace ProTools
{
    public class WebHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static NameValueCollection GETInput()
        {
        ...
        public static string GetFormString(string key, string defaultValue)
        {
            ...
            {
                NameValueCollection parmas = WebHelper.GetParmList(GetPostStr());
                value = parmas[key];
            }
            else
            {
        ...
        public static NameValueCollection GetParmList(string data)
        {
            ...
            if (!string.IsNullOrEmpty(data))
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    parmList[key] = HttpUtility.UrlDecode(value);
        ...
        /// <param name="postData">发送数据</param>
        /// <returns></returns>
        public static string GetHXRequestData(string url, string method, string token, bool isaddtoken, string postData = "")
        {
            return GetHXRequestData(url, method, postData, token, isaddtoken, Encoding.UTF8, 20000);
        }
        /// <summary>
        /// 获得http请求数据 （用户环信聊天室）
        /// </summary>
        ...
        public static string GetHXRequestData(string url, string method, string postData, string token, bool isaddtoken, Encoding encoding, int timeout)
        {
            if (!(url.Contains("http://") || url.Contains("https://")))
                url = "http://" + url;
